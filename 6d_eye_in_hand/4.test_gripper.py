# -*- coding: utf-8 -*-
"""
使用 Orbbec Gemini 2 相机和 ArUco 标记进行物体抓取测试（眼在手上 / eye-in-hand）

• 眼在手上：相机固定在机械臂末端，随臂运动
• 在观察位采集图像的同时记录当下的 T_end2base（末端->基座），用
      P_base = T_end2base @ T_camera2end @ P_camera
  把相机坐标转到基座坐标
• 使用 ArUco 标记进行物体定位
• 用于验证 3.calibrate.py 标定出的 T_camera2end 是否准确

使用方法（在 6d_eye_in_hand 目录下运行）:
    ../.venv/bin/python 4.test_gripper.py  # 运行抓取测试程序

与眼在手外（6d/）的区别：
    眼在手外相机不动，直接 P_base = T_camera2base @ P_camera；
    眼在手上相机随末端动，必须用观察位当下的 T_end2base 串联标定得到的 T_camera2end。
"""
...
class Calibration:
    ...
    def get_aruco_center(self, calib=True):
        ...
        Returns:
            tuple: (图像, 中心点坐标)
                - 图像: 包含颜色帧和深度帧的组合图像
                - 中心点坐标: 如检测到则返回[x, y, z]列表（相机系，米），否则为None
        """
        # 获取相机帧（深度已对齐到彩色）
    ...
    def run_recog(self):
        ...
        # 移动到观察位（相机随末端，俯视工作区）
        self.robot.move_xyz_rotation(self.observe_xyz, self.observe_rpy, rotation_order="xyz", speed_ratio=1)
        time.sleep(1)

        # ───── 主循环 ─────
        while True:
            # 移动到观察位置（相机在末端，必须先回到固定观察位再采集）
            self.robot.move_xyz_rotation(self.observe_xyz, self.observe_rpy, rotation_order="xyz", speed_ratio=1)
            time.sleep(1)

            # 记录采集瞬间的末端->基座位姿（眼在手上坐标变换的关键）
            T_end2base = self.robot.get_T()
            if T_end2base is None:
                print("无法获取末端位姿，重试")
                time.sleep(1)
                continue
