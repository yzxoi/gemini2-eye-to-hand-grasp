# -*- coding: utf-8 -*-
"""
使用 Orbbec Gemini 2 相机和棋盘格进行手眼标定（眼在手外 / eye-to-hand）

• 分辨率：1280 × 720 @ 30 FPS
• 眼在手外：相机固定在基座系，棋盘格固定在机械臂末端
• 使用棋盘格估计每张图的 board->camera 位姿，结合 end->base 做手眼标定
• 支持多种手眼标定算法：HORAUD、TSAI、PARK
• 输出「相机到基座」的刚体变换 T_camera2base.yaml（单位：mm）
...
class Calibration:
    ...
    def run_calibration(self, image_path, method=cv2.CALIB_HAND_EYE_HORAUD):
        ...
        # 眼在手外（eye-to-hand）：相机固定在基座系，棋盘格固定在末端。
        # OpenCV calibrateHandEye 原生求解眼在手上(cam2gripper)；眼在手外的标准做法是
        # 把「末端->基座」取逆得到「基座->末端」喂进去，返回的 X 即为「相机->基座」cam2base。
        R_base2ends = []
        t_base2ends = []
        for T_end2base in T_list:
            T_base2end = np.linalg.inv(T_end2base)
            R_base2ends.append(T_base2end[:3, :3])
            t_base2ends.append(T_base2end[:3, 3])

        # 执行手眼标定（眼在手外，得到相机 -> 基座）
        self.R_camera2base, self.t_camera2base = cv2.calibrateHandEye(
        ...
        self.T_camera2base[:3, :3] = self.R_camera2base
        self.T_camera2base[:3, 3] = self.t_camera2base.reshape(3)
        ...
        print("\n=== 手眼标定结果（眼在手外）===")
        print("相机到基座的旋转矩阵:")
        print(self.R_camera2base)
        print("相机到基座的位移向量:")
        print(self.t_camera2base, "（单位：mm）")

        # 保存标定结果（相机 -> 基座 刚体变换，单位 mm）
        T_data = {'T_camera2base': self.T_camera2base.tolist()}
        with open('./T_camera2base.yaml', 'w') as f:
            yaml.dump(T_data, f)
...
if __name__ == "__main__":
    ...
    print("\n相机到基座的旋转矩阵:")
    print(cali.R_camera2base)
    print("相机到基座的位移向量:")
    print(cali.t_camera2base, "（单位：mm）")
    print("相机到基座的齐次变换矩阵:")
    print(cali.T_camera2base)

    # 保存标定结果（相机 -> 基座）
    T_data = {'T_camera2base': cali.T_camera2base.tolist()}
    with open('./T_camera2base.yaml', 'w') as f:
        yaml.dump(T_data, f)
