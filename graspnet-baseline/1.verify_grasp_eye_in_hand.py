...
# 眼在手上（eye-in-hand）版 GraspNet 抓取验证脚本
#
# 与眼在手外版 1.verify_grasp.py 的区别仅在坐标变换：
#   • 标定文件读取 6d_eye_in_hand/T_camera2end.yaml（相机->末端，固定）
#   • 相机随末端运动，采集时记录当下的 T_end2base（末端->基座）
#   • T_grasp2base = T_end2base @ T_camera2end @ T_grasp2camera
#   • 相机在基座系的高度（高度过滤用）按 (T_end2base @ T_camera2end)[:3,3] 实时计算
#
# 运行（在 graspnet-baseline 目录下）：
...
class GraspNetDemo:
    def __init__(self):
        ...
        # 观察位（相机在末端，移到此处俯视工作区采集；此姿态的 T_end2base 参与坐标变换）
        self.observe_xyz = [260, 0, 400]
        self.observe_rpy = [180, 0, 90]
        ...
        # 眼在手上：加载固定的「相机->末端」变换（由 6d_eye_in_hand/3.calibrate.py 标定，单位 mm）
        T_path = os.path.join(os.path.dirname(ROOT_DIR), '6d_eye_in_hand', 'T_camera2end.yaml')
        ...
        # 移动到观察位（相机随末端，俯视工作区）
        result = self.EpRobot.move_xyz_rotation(self.observe_xyz, self.observe_rpy, rotation_order="xyz", speed_ratio=1)
        time.sleep(result)
        ...
        # 采集瞬间的末端->基座位姿，及由它推出的相机在基座系的位置（高度过滤用）
        # 在 get_one_frame() 中按当前观察位实时刷新
        self.T_end2base = None
    ...
    def get_one_frame(self):
        # 眼在手上：先回到观察位，再采集图像并记录当下的 T_end2base
        result = self.EpRobot.move_xyz_rotation(self.observe_xyz, self.observe_rpy, rotation_order="xyz", speed_ratio=1)
        if isinstance(result, (int, float)) and result > 0:
            time.sleep(result)
        time.sleep(0.5)

        # 记录采集瞬间的末端->基座位姿（眼在手上坐标变换的关键）
        T_end2base = self.EpRobot.get_T()
        while T_end2base is None:
            T_end2base = self.EpRobot.get_T()
        self.T_end2base = T_end2base
        # 相机在基座系的位置（高度过滤用），随观察位实时变化
        self.t_camera2base = (self.T_end2base @ self.T_camera2end)[:3, 3]
    ...
    def vis_grasps(self,  gg, cloud):
        ...
        def calculate_transforms(grasp_idx=0):
            ...
            # 眼在手上：抓取坐标系 -> 相机 -> 末端 -> 基座
            # 用采集该帧时记录的 T_end2base 串联固定的 T_camera2end
            T_grasp2base = self.T_end2base @ self.T_camera2end @ T_grasp2camera
